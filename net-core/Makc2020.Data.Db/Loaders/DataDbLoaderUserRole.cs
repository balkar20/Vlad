...
{
    /// <summary>
    /// Данные. База данных. Загрузчики. Сущность "UserRole".
    /// </summary>
    public class DataDbLoaderUserRole : DataBaseLoaderUserRole, ICoreBaseDataDbLoader
    {
        #region Constructors
        ...
        public DataDbLoaderUserRole(DataBaseObjectUserRole data = null)
        ...
        public async Task<int> LoadDataFrom(DbDataReader source, int ordinal, HashSet<string> props = null)
        {
            ...
            {
                Data.UserId = await source.GetFieldValueAsync<long>(ordinal++).CoreBaseExtTaskWithCurrentCulture(false);
            }

            if (props.Contains(nameof(Data.RoleId)))
            {
                Data.RoleId = await source.GetFieldValueAsync<long>(ordinal++).CoreBaseExtTaskWithCurrentCulture(false);
            }
